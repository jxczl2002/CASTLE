...
NUM_LAYERS = 3                      
DIR_A1 = f'3D_{NUM_LAYERS}_xy_test/condition'  
DIR_B1 = f'3D_{NUM_LAYERS}_xy_test/temp'      

AE_MODEL_DIR = 'Models_Autoencoders_Final'
MPNN_MODEL_PATH = 'Models_Flux_Specialized_Flux_Training_Data_Final_3D_3_xy/best_transfer_mpnn.pth' 
OUTPUT_DIR = f'Validation_Results_N1_{NUM_LAYERS}L_StartT1_Transfer_xy' 
...
def batch_encode(encoder, arr_flat, v_min, v_range):
    all_z = []
    for i in range(0, arr_flat.shape[0], EVAL_BATCH_SIZE):
        batch_tensor = normalize_t(arr_flat[i : i + EVAL_BATCH_SIZE], v_min, v_range)
        all_z.append(encoder.forward_encoder(batch_tensor))
    return torch.cat(all_z, dim=0)

def validate_N1_autoregressive():
    print("\n" + "="*70)
    print(f"[N1 Validation] 启动 N1 模型全量自回归多维指标推演引擎 (流式版)")
    ...
    for case_id in tqdm(case_folders, desc="Cases Validation"):
        ...
        with torch.no_grad():
            flat_K = extract_subs_vectorized(arr_K).reshape(-1, 16, 16, 16)
            Z_K = batch_encode(enc_K, flat_K, min_K, range_K).view(SX, SY, SZ, 32)
            
            flat_rhoCp = extract_subs_vectorized(arr_rhoCp).reshape(-1, 16, 16, 16)
            Z_rhoCp = batch_encode(enc_rhoCp, flat_rhoCp, min_rhoCp, range_rhoCp).view(SX, SY, SZ, 32)
            
            flat_Q = extract_subs_vectorized(arr_Q).reshape(-1, 16, 16, 16)
            Z_Q = batch_encode(enc_Q, flat_Q, min_Q, range_Q).view(SX, SY, SZ, 32)
            
            Z_true_list = []
            for t_idx in range(num_t):
                sub_t = extract_subs_vectorized(arr_Temp[t_idx]).reshape(-1,16,16,16)
                z_t = batch_encode(enc_Temp, sub_t, min_T, range_T).view(SX, SY, SZ, 32)
                Z_true_list.append(z_t)
            Z_true_all = torch.stack(Z_true_list)
            
            T_0_const = np.full((NX, NY, NZ), T_INIT_PHYSICAL, dtype=np.float32)
            flat_T_0 = extract_subs_vectorized(T_0_const).reshape(-1, 16, 16, 16)
            Z_0 = batch_encode(enc_Temp, flat_T_0, min_T, range_T).view(SX, SY, SZ, 32)
            ...
            for t in tqdm(range(1, num_t), leave=False, desc=f"{case_id} 滚动自回归推演"):
                ...
                for i in range(0, X_t.size(0), EVAL_BATCH_SIZE):
                    X_batch = X_t[i : i + EVAL_BATCH_SIZE]
                    z_pred_batch = mpnn(X_batch)
                    Z_pred_flat_list.append(z_pred_batch)
                    
                    t_pred_norm_batch = enc_Temp.forward_decoder(z_pred_batch).squeeze(1).cpu().numpy()
                    T_pred_norm_list.append(t_pred_norm_batch)
        ...
        del arr_K, arr_rhoCp, arr_Q, arr_Temp, pred_3D_frames, true_3D_frames, pred_4D_array, Z_true_all, flat_K, flat_rhoCp, flat_Q
