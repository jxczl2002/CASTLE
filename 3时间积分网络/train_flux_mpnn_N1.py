...
FLUX_DATA_DIR = 'Flux_Training_Data_Final'   
OUTPUT_DIR = 'Models_Flux_N1'             
...
print(f"Using device: {device} | N1 System (Strict Val_S={VAL_S} Benchmarking)")
...
def format_to_neighbors_batched(Z_b, B, sz):
...
def get_patched_X_batched(X_base, past_preds_list, B, V, sz):
    ...
    for i, z_pred in enumerate(reversed(past_preds_list)):
        ...
        z_neighbors = format_to_neighbors_batched(z_pred, B, sz) 
...
def train_flux_mpnn_N1():
    print(f"\n{'='*75}\n[N1 System] Pure Latent Multi-step Autoregressive Training\n{'='*75}")
    pt_files = glob.glob(os.path.join(FLUX_DATA_DIR, '*.pt'))
    if len(pt_files) == 0: return print("Error: No preprocessed flux data found.")
        
    random.seed(42); random.shuffle(pt_files)
    split_idx = int(len(pt_files) * 0.9)
    ...
    print(f"[-] 自动探测：当前数据属于 {NUM_LAYERS_DETECTED} 层芯片架构 (SZ={SZ_temp})")
    
    model = N1_FiLM_MPNN().to(device)
    optimizer = optim.AdamW(model.parameters(), lr=START_LR_MPNN, weight_decay=WEIGHT_DECAY)
    ...
    ckpt_path = os.path.join(OUTPUT_DIR, 'latest_N1_mpnn.pth')
    log_path = os.path.join(OUTPUT_DIR, 'N1_loss_log.txt')
    ...
    if os.path.exists(ckpt_path):
        ...
        print(f"[✓] 恢复断点成功。将从 Epoch {start_epoch} 继续进行后续推演。")
    else:
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for i in range(train_chunks):
            ...
            for bx, by in loader:
                ...
                for s in range(S):
                    X_base = bx[:, s] 
                    X_input = get_patched_X_batched(X_base, past_preds, B, V, SZ).reshape(B*V, 6924)
                ...
                pbar_train.update(1)
                pbar_train.set_postfix({'Loss': f"{train_loss_accum/samples:.10f}"})
        ...
        with open(log_path, 'a') as f:
            f.write(f"{epoch},{current_lr_mpnn:.1e},{S},{train_loss_ep:.10f},{val_loss_ep:.10f}\n")
        ...
        if val_loss_ep < best_val_loss:
            ...
            save_checkpoint_atomic(ckpt_state, os.path.join(OUTPUT_DIR, 'best_N1_mpnn.pth'))
